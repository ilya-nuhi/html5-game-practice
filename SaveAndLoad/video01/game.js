...
class Game{
	...
	init(){
		...
		const game = this;
		const strs = ["LOAD", "HARD", "MEDIUM", "EASY"];
		const suits = [0,4,2,1];
		
		const buttonoptions = {
			font: 'Verdana',
			fontSize: rect.h*0.6,
			onClick: function(){ game.showLogin(); }
		}
		
		for(let i=0; i<strs.length; i++){
			buttonoptions.text = strs[i];
			buttonoptions.rect = new Rect(rect.x, this.canvas.height - margin - (gap/3)*i - rect.h, rect.w, rect.h);
            if (i==1) buttonoptions.onClick = function(){ game.initGame(this); };
    ...
    login(){
        ...
        const game = this;
        $.getJSON('api.php', options, function(data){
            if (data.success){
        ...
        if (this.userId!==null){
            ...
            for(let pile of this.tableau){
                ...
                for(let card of pile){
                    if (card._index!=53 && faceUpIndex==undefined) faceUpIndex = index;
                    tmp.push(card.card);
                    index++;
                }
                if (faceUpIndex==undefined) faceUpIndex = index;
                if (tmp.length>0) tmp.push(faceUpIndex);
                tableau.push(tmp);
            }
            let foundation = [];
            for(let card of this.foundation){
                foundation.push(card._index);
            }
            let stack = [];
            for(let card of this.stack){
                stack.push(card.card);
            }
            const gameState = {
                tableau: tableau,
    ...
    load(){
        if (this.userId!==null){
            ...
            $.getJSON('api.php', options, function(data){
                if (data.success){
                    game.initGameFromLoad(data.json);
                }else{
                    console.log(data.error);
                }
            });
    ...
    initGameFromLoad(gameState){
		...
		for(let i=0; i<8; i++){
			cardoptions.x = left + i*width;
			let sprite = new Sprite("card", cardoptions);
            sprite.index = gameState.foundation[i];
            if (sprite.index!=54) sprite.opacity = 1;
			this.foundation.push(sprite);
		}
		...
		for(let col=0; col<10; col++){
			cardoptions.x = left + col*width;
			let pile = [];
            let tabcol = gameState.tableau[col];
            if (tabcol.length==0) continue;
            let faceUpIndex = tabcol.pop();
            let yPos = top;
			for(let row=0; row<tabcol.length; row++ ){
				cardoptions.y = yPos;
				if (row>=faceUpIndex){
					cardoptions.index = tabcol[row];
                    yPos += height * 3;
				}else{
					cardoptions.index = 53;
                    yPos += height;
				...
				const sprite = new Sprite('card', cardoptions);
				sprite.card = tabcol[row];
				sprite.cardName = this.cardName(tabcol[row]);
				pile.push(sprite);
			}	
			tableau.push(pile);
		}
		...
		cardoptions.index = 53;
		this.stack = [];
		for(let index of gameState.stack){
			const sprite = new Sprite('card', cardoptions);
			sprite.card = index;
			sprite.cardName = this.cardName(index);
			this.stack.push(sprite);
		}
